import { AnalysisResult, GeneratedSDK, GeneratedFile } from '../types';
import { logger } from '../utils/logger';

export class SDKGeneratorService {
  async generateSDK(analysis: AnalysisResult, providerName: string): Promise<GeneratedSDK> {
    logger.info(`Generating SDK for ${providerName} (${analysis.providerType})`);
  ...
  private generateIndexFile(providerName: string): GeneratedFile {
    return {
      path: 'src/index.ts',
      type: 'typescript',
...
export * from './interfaces';`
  ...
  private generateClientSDKFile(providerName: string, analysis: AnalysisResult): GeneratedFile {
    ...
    const imports = services.map(service => 
      `import { ${service} } from './services/${service.toLowerCase().replace('service', '')}.service';`
    ).join('\n');

    const properties = services.map(service => 
      `  public readonly ${service.replace('Service', '')}: ${service};`
    ).join('\n');

    const initialization = services.map(service => 
      `    this.${service.replace('Service', '')} = new ${service}(this.token);`
    ).join('\n');
...
${initialization}
  ...
  private generateAppConfigFile(): GeneratedFile {
    return {
...
export function getAppConfig(): AppConfig {
  ...
  private generateHttpServiceFile(): GeneratedFile {
    return {
...
export class HttpService {
  ...
  private generateLoggerFile(): GeneratedFile {
    return {
...
export const logger = pino({
  ...
  private generateErrorFile(): GeneratedFile {
    return {
...
export function createError(message: string, code?: string, statusCode?: number): SDKError {
  ...
  private generateAuthServiceFile(analysis: AnalysisResult): GeneratedFile {
...
export async function authenticate(apiKey: string, apiSecret?: string): Promise<string> {
  ...
  private generatePaymentServiceFile(analysis: AnalysisResult, providerName: string): GeneratedFile {
...
export class CheckoutService {
  ...
  private generateShippingServiceFile(analysis: AnalysisResult, providerName: string): GeneratedFile {
...
export class ShippingService {
  ...
  private generatePaymentInterfacesFile(): GeneratedFile {
    return {
...
export interface PaymentData {
  ...
  private generateShippingInterfacesFile(): GeneratedFile {
    return {
...
export interface ShipmentUpdateDTO {
  ...
  private generateBaseInterfacesFile(): GeneratedFile {
    return {
...
export interface PaginatedResponse<T = any> {
  ...
  private generateTestFile(providerName: string, providerType: string): GeneratedFile {
...
describe('ClientSDK', () => {
  ...
  private generateJestConfigFile(): GeneratedFile {
    return {
  ...
  private generateEslintConfigFile(): GeneratedFile {
    return {
      path: '.eslintrc.json',
      type: 'json',
      content: `{
  ...
  private generatePackageJson(normalizedName: string, providerName: string): any {
    return {
  ...
  private generateReadme(providerName: string, analysis: AnalysisResult): string {
    const normalizedName = this.normalizeProviderName(providerName);
